...
- [Library 3: tmux\_utils1.sh](#library-3-tmux_utils1sh)
  ...
  - [TMUX Script Execution Methods](#tmux-script-execution-methods)
    - [1. Direct Function Mode (Recommended)](#1-direct-function-mode-recommended)
    - [2. Embedded Mode](#2-embedded-mode)
    - [3. Script Mode](#3-script-mode)
...
3. **tmux_utils1.sh**: A utility library for managing tmux sessions from shell scripts, providing multiple approaches for executing scripts in tmux panes.
...
`tmux_utils1.sh` is a utility library for managing tmux sessions from shell scripts, providing multiple approaches for executing scripts in tmux panes.

## Key Features

- Create and manage tmux sessions programmatically
- Execute scripts in tmux panes using three different methods
- Support for variable sharing between parent script and tmux sessions
...
### 1. Direct Function Mode (Recommended)

Normal shell functions executed directly in a tmux pane.

```bash
# Define a normal shell function
monitor_files() {
    # Real commands that execute directly
    msg_bg_cyan "Monitoring files in: $(pwd)"
    while true; do
...
execute_shell_function "${SESSION_NAME}" 0 monitor_files "VARS_TO_EXPORT"
```
...
- See `Samples/tmux_direct_functions.sh` for examples

### 2. Embedded Mode

Inline scripts with heredoc syntax.

```bash
# Using heredoc for inline scripts
execute_script "${SESSION_NAME}" 0 "VARS_TO_EXPORT" <<'EOF'
...
- See `Samples/test_tmux1.sh` for examples
...
execute_function "${SESSION_NAME}" 0 welcome_script "VARS_TO_EXPORT"

# Or from a file
execute_file "${SESSION_NAME}" 0 "/path/to/script.sh" "VARS_TO_EXPORT"
```

**Benefits:**
- Good for reusable script templates
- Can organize scripts in separate files
- See `Samples/tmux_script_functions.sh` for examples

## Core Functions

| Function | Description |
|----------|-------------|
| `create_tmux_session [session_name]` | Create a new tmux session |
| `execute_in_pane [session] [pane] [command]` | Execute command in a pane (legacy method) |
| `execute_script [session] [pane] [vars] <<EOF ...` | Execute script using heredoc |
| `execute_function [session] [pane] [func_name] [vars]` | Execute script from generator function |
| `execute_file [session] [pane] [file_path] [vars]` | Execute script from file |
| `execute_shell_function [session] [pane] [func_name] [vars]` | Execute shell function directly |
| `create_new_pane [session] [split_type="h"]` | Create a new pane (h=horizontal, v=vertical) |
| `kill_tmux_session [session]` | Kill a tmux session and clean up resources |

## Sample Files

- `tmux_utils1.sh` - Core functions for managing tmux sessions
- `Samples/tmux_direct_functions.sh` - Examples of direct function mode
- `Samples/test_tmux1.sh` - Examples of embedded mode
- `Samples/tmux_script_functions.sh` - Examples of script generator functions
- `Samples/tmux_embedded_scripts.sh` - Examples of embedded script content
...
# Method 1: Run a direct shell function
monitoring() {
    msg_bg_green "Monitoring system"
    top -bn 1
    sleep 5
}
execute_shell_function "${SESSION_NAME}" 0 monitoring

# Method 2: Run an embedded script
execute_script "${SESSION_NAME}" 0 "SESSION_NAME" <<'EOF'
msg_header "Welcome to ${SESSION_NAME}"
echo "Current working directory: $(pwd)"
EOF

# Method 3: Run a script generator function
welcome_msg() {
    cat <<EOF
msg_bg_blue "Hello from tmux session ${SESSION_NAME}"
EOF
}
execute_function "${SESSION_NAME}" 0 welcome_msg "SESSION_NAME"
```
