...
There are two main ways to run scripts in tmux panes:
...
execute_function "${SESSION_NAME}" 0 welcome_script "SESSION_NAME DATA_FILE"
...
### Special Case: Direct Function Mode

For complex cases, you can use direct shell functions:
...
execute_shell_function "${SESSION_NAME}" 0 monitor "COUNT_FILE"
execute_shell_function "${SESSION_NAME}" 1 counter "COUNT_FILE"
```
...
# Blue counter function - increments by 3
blue() {
    while true; do
        v=$(($(cat ${COUNT_BLUE}) + 3))
        echo ${v} > ${COUNT_BLUE}
...
main() {
    s=$(create_tmux_session "counter_$(date +%s)")
