#!/usr/bin/bash
# test_param_handler_format.sh - Parameter format, export, and display tests for param_handler.sh
...
# Test 5: Simple Handle with Standard Option Names (fixed)
setup_test "TEST 5: Simple Handle with Standard Option Names (fixed)"

# Define parameters with standard option names
declare -A SIMPLE_PARAMS=(
    ["name:TEST_NAME"]="Person's name"                # Standard format
    ["age:TEST_AGE"]="Person's age"                   # Standard format
    ["email:TEST_EMAIL"]="Email address"              # Standard format
)
...
setup_test "TEST 6: Parameter Types"
