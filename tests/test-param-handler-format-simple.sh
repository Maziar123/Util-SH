...
original_output=$(param_handler::print_summary)
echo "DEBUG: Original STDOUT:"
echo ">>>$original_output<<<"

stripped_colors=$(echo "$original_output" | sed 's/\x1b\[[0-9;]*[mK]//g')
echo "DEBUG: After stripping colors:"
echo ">>>$stripped_colors<<<"

stripped_trailing_ws=$(echo "$stripped_colors" | sed 's/[[:space:]]*$//')
echo "DEBUG: After stripping trailing whitespace:"
echo ">>>$stripped_trailing_ws<<<"

actual_output=$(echo "$stripped_trailing_ws" | sed '/^$/d')
echo "DEBUG: Final actual_output (after removing blank lines):"
echo ">>>$actual_output<<<"

# Define expected output for comparison
expected_summary="Parameter Summary:
Named parameters: 2
Positional parameters: 1
Total parameters: 3"

# Assert that the processed output equals the expected output
run_test "Parameter summary format" "$actual_output" "$expected_summary"
