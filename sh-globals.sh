...
_MAIN_SCRIPT_NAME=""
get_script_name() {
  # Return the captured main script name if available
  if [[ -n "$_MAIN_SCRIPT_NAME" ]]; then
    echo "$_MAIN_SCRIPT_NAME"
    return
  fi
  
  # Fallback logic (useful if sh-globals.sh is executed directly)
  if [[ -n "${BASH_SOURCE[0]}" ]]; then
...
sh-globals_init() {
  # Capture the main script name *before* doing anything else
  # Use $0 from the perspective of the calling script
  _MAIN_SCRIPT_NAME="$(basename "$0")"
...
export SH_GLOBALS_LOADED _MAIN_SCRIPT_NAME
